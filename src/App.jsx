  ...
  return (
    <div style={{ background: C.bg1, minHeight: "100vh" }}>
      ...
      <div style={{ background: `linear-gradient(180deg, ${C.bg0} 0%, ${C.bg1} 100%)`, borderBottom: `1px solid ${C.border}`, padding: "40px 16px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 360px", gap: 40, alignItems: "center" }} className="sk-hero-grid">
          <div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 24 }} className="sk-content-grid">
  ...
  return (
    <div style={{ background: C.bg1, minHeight: "100vh", padding: "24px 16px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        {/* Prize bar */}
        <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px", marginBottom: 20, display: "flex", gap: 16, overflowX: "auto", scrollbarWidth: "none" }}>
          {PRIZE_TIERS.map(t => (
            <div key={t.pos} style={{ textAlign: "center", flexShrink: 0, minWidth: 90 }}>
              <div style={{ color: C.grey1, fontSize: 11, marginBottom: 3 }}>{t.pos}</div>
        ...
        {step === "done" && (
          <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 14, padding: 40, textAlign: "center", marginTop: 20 }}>
            ...
            <div style={{ background: C.bg0, borderRadius: 10, padding: "16px 20px", marginBottom: 24, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
    ...
    <div style={{ background: C.bg1, minHeight: "100vh", padding: "24px 16px" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "48px 1fr 100px 100px", padding: "10px 16px", borderBottom: `1px solid ${C.border}`, background: C.bg0 }}>
            {["#", "Player", "Points", "Prize"].map(h => <span key={h} style={{ color: C.grey2, fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>{h}</span>)}
          </div>
          {LEADERBOARD.map((u, i) => (
            <>
              <div key={u.rank} style={{ display: "grid", gridTemplateColumns: "48px 1fr 100px 100px", padding: "13px 16px", borderBottom: `1px solid ${C.border}`, background: u.rank <= 3 ? "rgba(26,86,219,0.06)" : "transparent", alignItems: "center" }}>
                <span style={{ color: u.rank === 1 ? "#fbbf24" : u.rank === 2 ? "#9ca3af" : u.rank === 3 ? "#cd7f32" : C.grey2, fontWeight: 800, fontSize: 14 }}>{u.rank}</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 16 }}>{u.country}</span>
                  <span style={{ color: C.white, fontWeight: 600, fontSize: 14 }}>{u.name}</span>
                </div>
                <span style={{ color: C.blue, fontWeight: 800, fontSize: 14 }}>{u.pts.toLocaleString()}</span>
                <span style={{ color: C.win, fontWeight: 700, fontSize: 13 }}>{u.prize}</span>
              </div>
